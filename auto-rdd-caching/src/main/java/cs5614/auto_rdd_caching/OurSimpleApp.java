...
public class OurSimpleApp {
     ...
     */
    private static Map<RDD<?>, Integer> job1(
        JavaSparkContext sc,
        boolean verbose) {
     ...
     *         actions called on it
     */
    private static Map<RDD<?>, Integer> job2(
        JavaSparkContext sc,
        boolean verbose) {
        // 1. Get csv data and filter out header
        // Verbose function
     ...
     *         actions called on it
     */
    private static Map<RDD<?>, Integer> job3(
        JavaSparkContext sc,
        boolean verbose) {
     ...
     *         actions called on it
     */
    private static Map<RDD<?>, Integer> job4(
        JavaSparkContext sc,
        boolean verbose) {
     ...
     *         actions called on it
     */
    private static Map<RDD<?>, Integer> job5(
        JavaSparkContext sc,
        boolean verbose) {
        JavaRDD<String> flightsData = DataReader.getFlights(sc);
        JavaPairRDD<String, Tuple3<String, String, String>> flightsTuples =
    ...
    public static void main(String[] args) {
        ...
        SparkConf conf = new SparkConf().setAppName("Our Simple App").setMaster(
            "local[4]"); // runs on 4 worker threads
        ...
        System.out.printf("%d executions of job4 takes %d nanoseconds%n",
            n_times, time);
